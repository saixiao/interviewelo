...
Four practice modes, each producing a score that feeds one unified Elo rating (0–3000) across ten named tiers (Noob → Intern → Entry Level → Mid Level → Senior → Staff → Sr. Staff → Principal → Fellow → AGI), so the user can track day-over-day progression toward being interview-ready.

Confirmed tech decisions:
- **Frontend**: React SPA (Vite + TypeScript), deployed to Vercel.
- **Backend**: Python/FastAPI — chosen specifically because the LC coding round needs to execute submitted Python against test cases, so keeping the backend in Python keeps that consistent.
...
- **Deployment**: cloud-hosted — Vercel doesn't suit a long-lived FastAPI process or multi-second LLM/sandbox calls, so the backend needs a dedicated host (Fly.io recommended, Render/Railway as alternatives) plus managed Postgres (Neon recommended).
...
│       ├── features/  # typing/, coding/, approach/, design/, dashboard/, auth/
...
│       ├── coding/    # problems, submissions, runner client
...
├── runner/            # isolated Python code-execution service (Phase 2)
...
- **DB**: Postgres + SQLAlchemy 2.0 + Alembic migrations. JSONB for flexible payloads (test cases, keystroke logs, LLM grade breakdowns) so early schema churn stays cheap.
...
Each session produces a normalized score **S ∈ [0, 1]** and the content it was attempted on has a **difficulty rating D** (e.g. easy problem D=800, medium D=1400, hard D=2000; typing packs get D from length/symbol density). Standard expected-outcome math:
...
- **LC coding**: S = fraction of hidden tests passed, weighted 0.85, +0.15 time bonus if solved under par time; 0 if nothing submitted.
...
**Overall Elo** = mean of the four category ratings (an unplayed category counts at the 500 floor, so the overall number is meaningful from day one and playing all four modes is incentivized). Every update writes an `elo_history` row for charting.

## Core Data Model

```
users            (id, email, password_hash, display_name, created_at)
user_ratings     (user_id, category ∈ {typing,coding,approach,design}, rating, sessions_count)
...
problems         (id, slug, title, difficulty, description_md, examples JSONB,
                  constraints_md, starter_code, test_cases JSONB[{input, expected, hidden}],
                  time_limit_ms, par_time_s)
submissions      (id, user_id, problem_id, code, status ∈ {pending,running,passed,failed,error,timeout},
                  tests_passed, tests_total, runtime_ms, error_output, score, elo_delta, created_at)
...
- Frontend: Vite + React Router + Tailwind; auth pages; app shell with four mode tiles (Human-Benchmark-style grid, greyed out until their phase ships); Elo badge in the header.
- Deploy both ends now so every later phase is a deploy, not a migration.

## Phase 1 — Typing Racer (first playable feature)

Self-contained: no LLM, no sandbox — validates session flow, scoring, Elo, and the results-reveal UX end to end.
...
## Phase 2 — LC Problems + Sandboxed Python Execution

The security-sensitive phase — isolate it fully from the main backend.

**Sandbox approach.** Running arbitrary user-submitted Python safely is the trickiest infra piece. Options considered: in-process `exec()` restrictions (trivially escapable — rejected), subprocess+rlimits inside the main backend (escape reaches DB creds and API keys — rejected). **Recommended**: a separate `runner/` FastAPI micro-service, deployed as its own container with no DB access, no secrets beyond a shared-secret header for backend→runner auth, and egress disabled at the platform level. Per run: `subprocess` with `resource.setrlimit` (CPU ~5s, memory ~256MB, no forks), 10s wall-clock kill, non-root user, read-only FS + tmpfs workdir, `python -I` isolated mode. The container is the real sandbox boundary — a worst-case escape lands in a credential-free, network-less box. (Upgrade path if needed later: per-run Docker/Firecracker isolation, or a hosted execution API like Judge0/Piston.)
- Runner API: `POST /run {code, test_cases[], time_limit_ms} → {results[], stdout, runtime_ms}`; a JSON-driven harness calls the user's `class Solution` method per test case and compares outputs.
- Backend: `GET /problems`, `GET /problems/{slug}`, `POST /problems/{slug}/submissions` (calls runner synchronously with a timeout), then scores + Elo. Hidden test cases never leave the server.
- Problem bank: 20–30 problems seeded from checked-in YAML/JSON (`seeds/problems/two-sum.yaml`, etc.) — description, examples, constraints, starter code, visible + hidden tests, difficulty, par time.
- Frontend: split-pane view — description left, CodeMirror 6 (Python mode) right; run-visible-tests + submit; per-test pass/fail; ResultsReveal with pass count, runtime, Elo delta.

## Phase 3 — Quick-Fire Approach Round + Claude Grading

First LLM integration; establishes the `llm/` grading pattern Phase 4 reuses.
...
## Phase 4 — System Design Round + Claude Follow-ups
...
- Same Anthropic client wrapper as Phase 3, so prompt-engineering iterations live in one place. Seed 10–15 design prompts each with a tailored rubric.
- Frontend reuses SessionShell/BigTimer/ResultsReveal; new pieces are the transcript view and follow-up bubbles.

## Phase 5 — Dashboard, Progression, Polish
...
- Polish: tier-up celebration animation, daily-practice prompt on the home screen ("Today: 1 typing sprint, 1 problem, 1 quick-fire"), settings page, mobile-usable typing fallback.
- Backlog (not in scope now): leaderboards, spaced repetition of failed problems, Elo decay.
...
3. **Results reveal**: screen wipes to one enormous animated number (net WPM / tests passed / grade), sub-stats fade in, then Elo delta slides in with tier badge (tier-up burst when crossed), then "Go again" / "Details" buttons.

## Deployment

- **Frontend**: Vercel (static SPA build; `VITE_API_URL` env).
- **Backend**: Fly.io (recommended for multi-app + egress control, which the runner needs) or Render/Railway as simpler alternatives. Dockerfile-based deploy, Alembic migrations on release.
- **Runner**: separate app/service (Phase 2), internal-only networking to the backend, egress blocked, no secrets.
- **Postgres**: Neon (free tier, branching) or the host's managed Postgres.
- **Secrets**: `DATABASE_URL`, `JWT_SECRET`, `ANTHROPIC_API_KEY` (backend only — never the runner, never the frontend), `RUNNER_SHARED_SECRET`.

## Risks

- **Sandbox escape** → mitigated by architecture (credential-free, egress-blocked runner container), not just rlimits.
...
- `runner/app/main.py` — the sandboxed Python execution service (the security-critical piece).
- `backend/app/llm/grader.py` — Anthropic client wrapper + structured-output graders for approach and design rounds.
- `frontend/src/components/ResultsReveal.tsx` — the shared big-number reveal that defines the game feel across all four modes.
...
- Phase 2: submit a known-correct and known-wrong solution to a seeded problem, confirm pass/fail and hidden tests aren't leaked to the client; attempt a deliberately malicious submission (e.g. infinite loop, large allocation) to confirm the runner kills it and the main backend is unaffected.
- Phase 3/4: run a real 10-minute session, confirm Claude's structured grade parses without validation errors and Elo updates sensibly.
- Phase 5: confirm dashboard charts reflect real `elo_history` rows across all four categories.
